define('mkt_depthchart', ['api_mkt'], function(api_mkt) {
	var depthchart = {
		interval: 1000, // 请求间隔
		timer: null, // 定时器
		stop: function() {
			clearTimeout(depthchart.timer);
		},
		onChange: updatebuy_sell, // 价格改变时回调, 传参(当前价格, 上一次价格, 改变大小)
	};
	var once = depthchart.once = function(callback) {
	...
	var updatebuy_sell_floor = function(haha){
		...
		console.log("en")
		        /**
         * chart: {type: "area", renderTo: "graphbox2",width:960},
            title: {text: ""},
            credits: {
                enabled: false
            },
            ...
            series: [{name:'Bid',data:[]},{name:'Ask',data:[]}]
         */
	...
	//交易大厅深度图的轮询
	var get = depthchart.get = function(){
		once(function(next){
			//updatebuy_sell_floor(next);
			depthchart.timer = setTimeout(depthchart.get, depthchart.interval);
		});
	}
	return depthchart;
});
